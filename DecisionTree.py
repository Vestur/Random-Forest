...
def get_attributes_and_categories(all_data: np.ndarray, label_column_name: int):
        ...
        while(invalid_response):
            if cont_response != 'yes' and cont_response != 'no':
                input("Please answer with 'yes' or 'no', are all attributes made of continous data?")
            else:
        ...
        if not cont:
            ...
            while(invalid_response):
                if cont_response != 'yes' and cont_response != 'no':
                    input("Please answer with 'yes' or 'no', are all attributes made of categorical data?")
                else:
                    if cont_response == 'yes':
                        cat = True
                    else:
...
def plot(df: np.ndarray, label_column_name: int):
...
def main():
    # uncomment to run code on house votes dataset
    df = load("datasets/hw3_house_votes_84.csv", ',')
    label_column_name = 16

    # uncomment to run code on wine dataset
    # df = load("datasets/hw3_wine.csv", '\t')
    # label_column_name = 0

    # uncomment to run code on cancer dataset
